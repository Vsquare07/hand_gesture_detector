...
model = torch.load(f="models/model5.pth", weights_only=False)
...
while cap.isOpened():
    ...
    if(len(landmarks)!=0):
        ...
        with torch.inference_mode():
            ...
            cv2.putText(img, classes[pred_idx], landmarks[0][:2], cv2.FONT_HERSHEY_TRIPLEX, 1, (255,255,255),1)
            str_probs = "dislike = "+str(int(pred_prob[0].item()*100))+"% like = "+ str(int(pred_prob[1].item()*100))+"%"
            cv2.putText(img, str_probs, (200,50), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0,0), 1)
    cTime = time.time()
